...
class VBPRC(GeneralRecommender):
    r"""BPR is a basic matrix factorization model that be trained in the pairwise way.
    """
    def __init__(self, config, dataloader):
        super(VBPRC, self).__init__(config, dataloader)
    ...
    def forward(self, dropout=0.0):
        ...
        noisy_item_embeddings = item_embeddings + self.noise_level * torch.randn_like(item_embeddings) # adding pertubations after multimodal fusion
    ...
    def calculate_loss(self, interaction):
        ...
        user_embeddings, item_embeddings, noisy_item_embeddings = self.forward() 
        ...
        loss = (1 - self.consistency_weight) * general_loss + self.consistency_weight * consistency_loss # final loss
        return loss

    def full_sort_predict(self, interaction):
        user = interaction[0]
        user_embeddings, item_embeddings, a = self.forward()
        user_e = user_embeddings[user, :]
        all_item_e = item_embeddings
