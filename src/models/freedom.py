...
class FREEDOM(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.noisy_mm_adj = None, None #NNN
        ...
        if self.v_feat is not None:
            self.image_embedding = nn.Embedding.from_pretrained(self.v_feat, freeze=False) #v_feat是预训练的图像特征
            self.image_trs = nn.Linear(self.v_feat.shape[1], self.feat_embed_dim) #所有特征统一
        ...
        if os.path.exists(mm_adj_file):
            ...
            self.noisy_mm_adj = self.mm_adj + self.noise_level * torch.randn_like(self.mm_adj) #NNN
        else:
            if self.v_feat is not None:
