...
class FREEDOMA(GeneralRecommender):
    def __init__(self, config, dataset):
        super(FREEDOMA, self).__init__(config, dataset)
        ...
        self.noise_level_net_t = NoiseLevelNetwork(input_dim=64) 
        self.noise_level_net_v = NoiseLevelNetwork(input_dim=64) 
    ...
    def calculate_loss(self, interaction):
        ...
        noisy_mf_v_loss, noisy_mf_t_loss = 0.0, 0.0 
        adaptive_noise_level_t, adaptive_noise_level_v = 0.0001, 0.0001 
        p = Normal(0, 0.5) 
        if self.t_feat is not None:
            text_feats = self.text_trs(self.text_embedding.weight)
            adaptive_noise_level_t = self.noise_level_net_t(text_feats) 
            noisy_text_feats = text_feats + adaptive_noise_level_t * torch.randn_like(text_feats) 
            sigma_t = adaptive_noise_level_t 
            sigma_t = torch.clamp(sigma_t, min=1e-15) 
            q_t = Normal(text_feats, sigma_t) 
            vib_loss_t = kl_divergence(q_t, p).mean()  
            vib_loss_t *= 0.001 
            mf_t_loss = self.bpr_loss(ua_embeddings[users], text_feats[pos_items], text_feats[neg_items])
            noisy_mf_t_loss = self.bpr_loss(ua_embeddings[users], noisy_text_feats[pos_items], noisy_text_feats[neg_items]) 
        if self.v_feat is not None:
            image_feats = self.image_trs(self.image_embedding.weight)
            adaptive_noise_level_v = self.noise_level_net_v(image_feats) 
            sigma_v = adaptive_noise_level_v 
            sigma_v = torch.clamp(sigma_v, min=1e-15)
            q_v = Normal(image_feats, sigma_v) 
            vib_loss_v = kl_divergence(q_v, p).mean()  
            vib_loss_v *= 0.001 
            noisy_image_feats = image_feats + adaptive_noise_level_v * torch.randn_like(image_feats) 
            mf_v_loss = self.bpr_loss(ua_embeddings[users], image_feats[pos_items], image_feats[neg_items])
            noisy_mf_v_loss = self.bpr_loss(ua_embeddings[users], noisy_image_feats[pos_items], noisy_image_feats[neg_items]) 

        #1. general loss
        general_loss = batch_mf_loss + self.reg_weight * (mf_t_loss + mf_v_loss + noisy_mf_t_loss + noisy_mf_v_loss)  
        #2. consistency loss
        consistency_loss_t, consistency_loss_v = 0.0, 0.0
        if self.t_feat is not None:
            text_feats = torch.softmax(text_feats, dim=-1)
            noisy_text_feats = torch.softmax(noisy_text_feats, dim=-1) 
            consistency_loss_t = self.js_divergence(text_feats, noisy_text_feats)
        if self.v_feat is not None:
            image_feats = torch.softmax(image_feats, dim=-1)
            noisy_image_feats = torch.softmax(noisy_image_feats, dim=-1) 
            consistency_loss_v = self.js_divergence(image_feats, noisy_image_feats)
        ...
        return loss + vib_loss_t + vib_loss_v
