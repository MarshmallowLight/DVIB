...
class MMGCNC(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MMGCNC, self).__init__(config, dataset)
    ...
    def forward(self):
        ...
        if self.v_feat is not None:
            v_feat_noisy = self.v_feat + self.noise_level * torch.randn_like(self.v_feat) 
            representation = self.v_gcn(self.v_feat, self.id_embedding) 
            noisy_representation = self.v_gcn(v_feat_noisy, self.id_embedding) 
        if self.t_feat is not None:
            t_feat_noisy = self.t_feat + self.noise_level * torch.randn_like(self.t_feat) 
