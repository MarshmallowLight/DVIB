...
class GRCNC(GeneralRecommender):
    def __init__(self,  config, dataset):
        super(GRCNC, self).__init__(config, dataset)
    ...
    def forward(self):
        ...
        noisy_weight = weight + self.noise_level * torch.randn_like(weight)
        ...
        if self.weight_mode == 'confid':
            ...
            noisy_weight = noisy_weight * confidence 
            noisy_weight, _ = torch.max(noisy_weight, dim=1) 
            noisy_weight = noisy_weight.view(-1, 1)
        ...
        noisy_id_rep = self.id_gcn(edge_index, noisy_weight) 


        if self.fusion_mode == 'concat':
            representation = torch.cat((id_rep, content_rep), dim=1)
            noisy_representation = torch.cat((noisy_id_rep, content_rep), dim=1) 
