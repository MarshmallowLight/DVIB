...
class MMGCNJSMN6(GeneralRecommender):
    def __init__(self, config, dataset):
        super(MMGCNJSMN6, self).__init__(config, dataset)
        ...
        self.noise_level_net_v = NoiseLevelNetwork(input_dim=4096,hidden_dim1 = 2048,hidden_dim2 = 1024) 
        self.noise_level_net_t = NoiseLevelNetwork(input_dim=384, hidden_dim1 = 192, hidden_dim2 = 96) 
        self.general = config['general']
        self.noise = config['noise'] 
        self.v_feat += self.noise * torch.randn_like(self.v_feat) 
        self.t_feat += self.noise * torch.randn_like(self.t_feat) 
    ...
    def forward(self):
        ...
        adaptive_noise_level_v, adaptive_noise_level_t = 0.0001, 0.0001 
        p = Normal(0, 0.5) 
        vib_loss_v, vib_loss_t = 0.000, 0.000 
        if self.v_feat is not None:
            adaptive_noise_level_v = self.noise_level_net_v(self.v_feat) 
            sigma_v = adaptive_noise_level_v 
            sigma_v = torch.clamp(sigma_v, min=1e-6) 
            q = Normal(self.v_feat, sigma_v) 
            vib_loss_v = 0.001 * kl_divergence(q, p).mean() 
            v_feat_noisy = self.v_feat + adaptive_noise_level_v * torch.randn_like(self.v_feat) 
            representation = self.v_gcn(self.v_feat, self.id_embedding) 
            noisy_representation = self.v_gcn(v_feat_noisy, self.id_embedding) 
        if self.t_feat is not None:
            adaptive_noise_level_t = self.noise_level_net_t(self.t_feat) 
            sigma_t = adaptive_noise_level_t * adaptive_noise_level_t
            sigma_t = torch.clamp(sigma_t, min=1e-6) 
            q = Normal(self.t_feat, sigma_t) 
            vib_loss_t = 0.001 * kl_divergence(q, p).mean() 
            t_feat_noisy = self.t_feat + adaptive_noise_level_t * torch.randn_like(self.t_feat) 
            if representation is None:
                representation = self.t_gcn(self.t_feat, self.id_embedding)
                noisy_representation = self.t_gcn(t_feat_noisy, self.id_embedding) 
            else:
                rep = self.t_gcn(self.t_feat, self.id_embedding) 
                noisy_rep = self.t_gcn(t_feat_noisy, self.id_embedding) 
                representation += rep 
                noisy_representation += noisy_rep 
        ...
        return representation, noisy_representation, vib_loss_t + vib_loss_v 
    ...
    def calculate_loss(self, interaction):
        ...
        # 1. general loss
        general_loss = loss + reg_loss * 2 + noisy_loss 
        # 2. consistency loss
        ...
        score = torch.softmax(score_matrix, dim=-1)  
        noisy_score = torch.softmax(noisy_score_matrix, dim=-1)  
